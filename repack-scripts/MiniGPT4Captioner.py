"""
Main Captioner class for using MiniGPT4 in general. `args` obtained through command line.
"""
...
class MiniGPT4Captioner():
    ...
    def generate_dataset(
        ...
        try:
            for i, dat in enumerate(tqdm(dataset)):
                ...
                if i % 50 == 0 and i > 0:
                    prefix = f'{file_prefix}_{half}' if half is not None else file_prefix
                    tmp_name = f"cached_{prefix}_{self.get_time_suffix()}.json"
            ...
            date = datetime.now()
            prefix = f'{file_prefix}_{half}' if half is not None else file_prefix
            outfile_name = f'{prefix}_{self.get_time_suffix()}.json'
