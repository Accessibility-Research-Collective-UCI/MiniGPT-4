...
parser.add_argument(
    "--res", type=float, default=100.0, help="resolution used in refcoco"
)
parser.add_argument(
    "--resample", action="store_true", help="resolution used in refcoco"
)
...
for dataset in args.dataset:
    for split in eval_dict[dataset]:
        ...
        with open(
            os.path.join(eval_file_path, f"{dataset}/{dataset}_{split}.json"), "r"
        ) as f:
            refcoco = json.load(f)
