...
if "okvqa" in args.dataset:
    ...
    for images, questions, question_ids, img_ids in eval_dataloader:
        ...
        for answer, question_id, question, img_id in zip(
            answers, question_ids, questions, img_ids
        ):
            result = dict()
    ...
    quesFile = os.path.join(
        eval_file_path, "OpenEnded_mscoco_val2014_questions_clean.json"
    )
...
if "iconvqa" in args.dataset:
    ...
    for images, texts, candidates, answers in tqdm(eval_dataloader):
        ...
        instructions = [
            "<s>[INST] <Img><ImageHere></Img> {} [/INST]".format(text) for text in texts
        ]
        answer_ranks = model.multi_select(
            images, instructions, candidates, num_cand=num_cand
        )
