...
class REFER:
    ...
    def getRefIds(self, image_ids=[], cat_ids=[], ref_ids=[], split=""):
        image_ids = image_ids if type(image_ids) is list else [image_ids]
        cat_ids = cat_ids if type(cat_ids) is list else [cat_ids]
        ref_ids = ref_ids if type(ref_ids) is list else [ref_ids]
    ...
    def loadRefs(self, ref_ids=[]):
        if type(ref_ids) is list:
            return [self.Refs[ref_id] for ref_id in ref_ids]
        elif type(ref_ids) is int:
            return [self.Refs[ref_ids]]

    def loadAnns(self, ann_ids=[]):
        if type(ann_ids) is list:
            return [self.Anns[ann_id] for ann_id in ann_ids]
        elif type(ann_ids) is int:
            return [self.Anns[ann_ids]]

    def loadImgs(self, image_ids=[]):
        if type(image_ids) is list:
            return [self.Imgs[image_id] for image_id in image_ids]
        elif type(image_ids) is int:
            return [self.Imgs[image_ids]]

    def loadCats(self, cat_ids=[]):
        if type(cat_ids) is list:
            return [self.Cats[cat_id] for cat_id in cat_ids]
        elif type(cat_ids) is int:
    ...
    def showRef(self, ref, seg_box="box"):
        ...
        if seg_box == "seg":
            ...
            color = []
            c = "none"
            if type(ann["segmentation"][0]) is list:
